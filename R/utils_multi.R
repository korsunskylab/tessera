...
#' @export
tiles_bind2 = function(aggs1, aggs2) {
    return(list(
...
tiles_bind = function(objs) {
    ...
    for (i in seq_len(naggs - 1)) {
        aggs_list[[merge_next[1]]] = tiles_bind2(aggs_list[[merge_next[1]]], aggs_list[[merge_next[2]]])
...
bind_objs = function(objs) {
    ...
    dmt = purrr::reduce(purrr::map(objs, 'dmt'), dmt_bind2)
