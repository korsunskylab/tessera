...
- field:
  ...
  spatial gradient in expression for each of `D` embedding dimensions at
  every point (or edge/triangle).

## Value

A list with elements `u` (`2` x `2` x `N`), `s` (`2` x `N`), and `v`
(`D` x `2` x `N`) containing the left singular vectors, singular values,
